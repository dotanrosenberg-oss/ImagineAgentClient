...
### GET /api/chats/:chatId/profile-pic

Fetch a fresh profile picture URL for a chat directly from WhatsApp and update it in the database. Requires **API Key** + **WhatsApp connected**.
...
> **Note:** `profilePicUrl` may be `null` if the contact/group has no profile picture or has restricted visibility.
...
Sync chats from WhatsApp into the local database (including profile picture URLs). Requires **API Key** + **WhatsApp connected**.
...
Send any file or media attachment. Requires **API Key** + **WhatsApp connected**.

Supports images, videos, audio, documents (PDF, Office, etc.), archives, and any other file type. MIME type is auto-detected from the file extension for 40+ common formats, with fallback to `application/octet-stream`.
...
| `sendAsDocument` | boolean | No | If `true`, sends as a document attachment preserving the original filename. Otherwise WhatsApp may render it inline (e.g. images/videos displayed in-chat). Default: `false` |

**Supported file types include (not exhaustive):**
- **Images:** jpg, jpeg, png, gif, webp, svg, bmp, tiff, ico
- **Video:** mp4, avi, mov, mkv, webm, flv, wmv, 3gp
- **Audio:** mp3, ogg, wav, aac, flac, m4a, wma, opus
- **Documents:** pdf, doc, docx, xls, xlsx, ppt, pptx, txt, csv
- **Archives:** zip, rar, 7z, gz, tar
- **Other:** json, xml, html, css, js, apk, exe, dmg
...
Send a poll to a chat. Requires **API Key** + **WhatsApp connected**.
...
| Field | Type | Required | Description |
|---|---|---|---|
| `chatId` | string | Yes | Recipient chat ID (individual or group) |
| `question` | string | Yes | The poll question (max 255 chars) |
| `options` | string[] | Yes | Array of answer options (2-12 items, each max 100 chars) |
| `allowMultipleAnswers` | boolean | No | Allow selecting multiple options. Default: `false` |
...
**Errors:**
- `400` — Validation error (missing fields, too few/many options)
- `503` — WhatsApp not connected
