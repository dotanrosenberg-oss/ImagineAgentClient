...
import type { ChatAction } from './groupActions'
import { getChatActions } from './groupActions'
...
interface Props {
  ...
  onClose: () => void
  onExecuteAction: (action: ChatAction, message: string, contextMessages: ContextMessage[]) => void
}

export default function ChatActionsPanel({ chatMessages, onClose, onExecuteAction }: Props) {
  const [actions, setActions] = useState<ChatAction[]>([])
  const [invokeAction, setInvokeAction] = useState<ChatAction | null>(null)
  const [invokeMessage, setInvokeMessage] = useState('')
  const [selectedMsgIds, setSelectedMsgIds] = useState<Set<string>>(new Set())

  useEffect(() => {
    getChatActions().then(setActions)
  }, [])
    ...
    <div className="group-actions-panel">
      ...
      {actions.length === 0 ? (
        <div className="gap-empty">
          ...
          <p className="gap-empty-hint">Go to Settings to add chat actions</p>
        </div>
      ) : (
